...
use flint_server::protocols::decision::DecisionParser;
use flint_server::tools::Tool;
use serde_json::json;
...
fn wrapper_schema_compiles_for_real_tools() {
    ...
    let _ = flint_generate::Grammar::from_schema(
        &flint_server::tools::wrapper_schema(&tools, true),
    )
    .unwrap();
    let _ = flint_generate::Grammar::from_schema(
        &flint_server::tools::wrapper_schema(&tools, false),
    )
    .unwrap();
}
