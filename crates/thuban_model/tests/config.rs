use thuban_model::config::{f64_field, req, u32_field, u32_list};
use thuban_model::ops::{check_gemm_dims, check_head_dim};
use serde_json::json;
...
fn gemm_dims_must_tile_to_16() {
    ...
    assert!(
        check_gemm_dims(&[(15, 64)]).is_err(),
        "N must be a multiple of 16"
    );
    assert!(
        check_gemm_dims(&[(16, 32)]).is_err(),
...
fn head_dim_bounds() {
    ...
    assert!(
        check_head_dim(512).is_ok(),
        "Gemma 4 global heads widen to 512"
    );
