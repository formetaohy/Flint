...
use thuban_architectures::llama;
use thuban_backend::Backend;
use thuban_checkpoint::{GgufWriter, open_checkpoint};
...
fn synth_llama() -> (std::path::PathBuf, Value) {
    let dir = std::env::temp_dir().join(format!("thuban-batch-{}", std::process::id()));
