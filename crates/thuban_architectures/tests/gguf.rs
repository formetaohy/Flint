...
use thuban_architectures::keymap::gguf_key;
use thuban_checkpoint::{Checkpoint, MetaVal, Metadata, RawTensor};
use serde_json::json;
...
fn llama_synthesis_reads_metadata_and_tensor_names() {
    ...
    let cfg =
        thuban_architectures::gguf::synthesize_config(&src, thuban_architectures::Family::Llama)
            .unwrap();
    assert_eq!(
        cfg,
        json!({
...
fn gemma_synthesis_adds_end_of_turn_to_eos() {
    ...
    let cfg =
        thuban_architectures::gguf::synthesize_config(&src, thuban_architectures::Family::Gemma)
            .unwrap();
...
fn qwen35_synthesis_reads_interval_layout() {
    ...
    let cfg = thuban_architectures::gguf::synthesize_config(&src, thuban_architectures::Family::Qwen35)
        .unwrap();
...
fn qwen35_synthesis_fails_fast() {
    ...
    assert!(
        thuban_architectures::gguf::synthesize_config(&src, thuban_architectures::Family::Qwen35)
            .is_err(),
