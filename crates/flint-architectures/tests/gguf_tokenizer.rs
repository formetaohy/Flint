...
use flint_architectures::gguf_tokenizer;
...
fn bpe_rebuild_keeps_ids_and_honors_merges() {
    let tok = gguf_tokenizer::from_metadata(&bpe_meta()).unwrap();
...
fn unigram_markers_encode_as_single_ids() {
    let tok = gguf_tokenizer::from_metadata(&unigram_meta()).unwrap();
...
fn unigram_rejects_malformed_metadata() {
    assert!(
        gguf_tokenizer::from_metadata(&mutate_meta("tokenizer.ggml.tokens", None)).is_err(),
        "missing tokens"
    );
    assert!(
        gguf_tokenizer::from_metadata(&mutate_meta("tokenizer.ggml.scores", None)).is_err(),
        "missing scores"
    );
    ...
    let err = gguf_tokenizer::from_metadata(&mismatch)
        .err()
        .expect("length mismatch fails");
    assert!(err.to_string().contains("mismatch"), "{err}");

    assert!(
        gguf_tokenizer::from_metadata(&mutate_meta("tokenizer.ggml.unknown_token_id", None))
            .is_ok(),
        "unk id is optional"
    );
}
...
fn from_source_rejects_non_gguf_checkpoints() {
    let err = gguf_tokenizer::from_source(&NoTokenizer::new())
        .err()
...
fn load_prefers_tokenizer_json_over_gguf_metadata() {
    ...
    let tok = gguf_tokenizer::load(Path::new(&dir), source.as_ref()).unwrap();
    assert_eq!(
