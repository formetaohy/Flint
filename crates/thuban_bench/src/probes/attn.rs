...
pub(super) fn attn_probe() -> Result<()> {
    ...
    let args = thuban_model::rows::row_meta(&backend);
    ...
    let pages = max_seq.div_ceil(thuban_kernel::PAGE_LEN);
    let table: Vec<u32> = (0..pages).collect();
    let block_table = thuban_tensor::Tensor::new(
    ...
    let run_flash = |backend: &mut Backend| -> Result<()> {
        let mut enc = backend.encoder().unwrap();
        {
            ...
            backend.dispatch(
                ...
                [m.div_ceil(thuban_kernel::ATTN_BR), nq, 1],
            )?;
        }
